...
load("H:/data/kwave/medium/2018_11_07_layer_medium/Layer_medium_boundary_1.0mm_ IMCL0%.mat")
load("H:/data/kwave/result/2018_12_13_layer_medium_various/boundary_1.0mm_IMCL0%/rfdata.mat")
load("H:/data/kwave/result/2018_12_13_layer_medium_various/boundary_1.0mm_IMCL0%/kgrid.mat")
% 音速値
v_fat = 1450;%[m/s]
v_muscle = 1580;%[m/s]
% IMCL割合
...
interp_sourcewave = hilbert(interp1(source_wave,linspace(1,num_sample,num_sample*4)','spline'));
interp_rfdata = zeros(num_sample*4,num_receiver);
for jj = 1:num_receiver
    interp_rfdata(:,jj) = hilbert(interp1(rfdata(:,jj),linspace(1,num_sample,num_sample*4),'spline'));
end
auto_correlation_rfdata = diag(abs(interp_rfdata' * interp_rfdata));
...
for  kk = 1:num_assumed_depth
    for ll = 1:num_assumed_SOS
        ...
        for ii = 1:num_receiver
            ...
            correlation(kk,ll) = correlation(kk,ll) + abs(interp_rfdata(:,ii)'*delay_sourcewave(:,ii)...
                /sqrt(auto_correlation_rfdata(ii,1)*auto_correlation_source_wave(ii,1)));
        end
        correlation(kk,ll) = abs(correlation(kk,ll)/num_receiver);
    end
    disp(kk)
end
