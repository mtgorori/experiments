...
reference_point_lowerlimit = zeros(2,num_echo_receiver,num_depth);%均質性評価のためのRFデータマスキングに使う
reference_point_upperlimit = zeros(2,num_echo_receiver,num_depth);%均質性評価のためのRFデータマスキングに使う
...
for kk = 1:length(rate_IMCL)
    ...
    for ii = 1:num_depth
        ...
        for jj = 1:length(target_element)
            ...
            tmp = focused_rfdata_amp(reference_point(2,target_element(1,jj),ii),target_element(1,jj),ii)/length(target_element);
            ...
            tmp2 = focused_rfdata_phase(reference_point(2,target_element(1,jj),ii),target_element(1,jj),ii);
