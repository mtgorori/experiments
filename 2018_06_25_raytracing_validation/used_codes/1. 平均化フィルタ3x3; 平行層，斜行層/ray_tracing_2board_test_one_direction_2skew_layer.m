...
v_water = 1500;
v_fat = 1480;
w_fat = (X+Y<0) & (Y>-t_size/2) & (X>-t_size/2);
w_water = not(w_fat);
...
xlabel('x方向[mm]')
ylabel('y方向[mm]')
% exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_03_sound_velocity_2skewlayers','png',[300,300]);
% 媒質表示2(屈折率分布)
figure;
...
xlabel('x方向[mm]')
ylabel('y方向[mm]')
% exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_03_relative_refractive_index_2skewlayers','png',[300,300]);
%% 単純照射法を用いた最速経路の推定
for ii = t_num/4
    pos_tr = [t_pos(1,ii), t_pos(2,ii)];%送信素子位置ベクトル
    pos_ray = pos_tr; %音線位置ベクトル
    num_ray_head = 1; %音線先頭更新回数
    theta0 = pi*(3/2); %初期射出角[rad]
    while(1)%音線作成ループ
    ...
    xlabel('x方向[mm]')
    ylabel('y方向[mm]')
%     exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_03_sound_ray_2layers_2_skewed','png',[300,300]);
end