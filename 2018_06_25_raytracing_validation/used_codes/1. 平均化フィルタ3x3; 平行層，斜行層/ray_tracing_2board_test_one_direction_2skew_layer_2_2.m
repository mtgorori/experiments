...
%% セル設定
% cell_num = 1024;%(2018-07-05まで)
cell_num = 2048;%(2018-07-06：誤差が生じた原因として計算グリッドの粗さが考えられるから)
...
ds = cell_size/4;%弧長（音線）の微小変化分
t_angle = atan2(t_pos(2,:), t_pos(1,:));%センサ位置角
%% 媒質設定
% 音速分布
v_water = 1500;
v_fat = 1480;
w_fat = (tan(pi/8)*X+Y<0) & (Y>-t_size/2) ;
w_water = not(w_fat);
...
xlabel('x方向[mm]')
ylabel('y方向[mm]')
% exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_03_sound_velocity_2skewlayers_2','png',[300,300]);
exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_06_sound_velocity_2skewlayers_2_2','png',[300,300]);
% 媒質表示2(屈折率分布)
figure;
...
xlabel('x方向[mm]')
ylabel('y方向[mm]')
% exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_03_relative_refractive_index_2skewlayers_2','png',[300,300]);
exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_06_relative_refractive_index_2skewlayers_2_2','png',[300,300]);
%% 単純照射法を用いた最速経路の推定
for ii = 38
    pos_tr = [t_pos(1,ii), t_pos(2,ii)];%送信素子位置ベクトル
    pos_ray = pos_tr; %音線位置ベクトル
    num_ray_head = 1; %音線先頭更新回数
    theta0 = pi*(3/2+1/4-1/8); %初期射出角[rad]
    while(1)%音線作成ループ
        ...
        nx = (n_cal(ix+1,jy)-n_cal(ix-1,jy))/2/cell_size;%音線をストップするトリガーは他にあるはず．コメントアウト
    ...
    axis tight;
    colorbar;
    caxis([min(min(n)) max(max(n))])
    c = colorbar;
    c.Label.String = '相対屈折率[水の屈折率=1.0]';
    xlabel('x方向[mm]')
    ylabel('y方向[mm]')
%     exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_03_sound_ray_2layers_2_skewed_2','png',[300,300]);
    exportfig('H:\result\2018_06_25_raytracing_validation\2018_07_06_sound_ray_2layers_2_skewed_2_2','png',[300,300]);
end