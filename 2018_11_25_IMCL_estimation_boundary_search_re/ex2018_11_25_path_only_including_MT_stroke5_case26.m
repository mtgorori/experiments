    ...
    for ll = 1:num_medium
        ...
        for kk = 1:num_rate_IMCL
            ...
            if kk == 1
                ...
                %% 反射強度が最大の焦点位置を探索．
                [max_tmp,ind_tmp] = findpeaks(abs(focal_signal_total(9:end,kk,ll,mm)),'Npeaks',2,'SortStr','descend');
...
%                 tmp2 = focused_rfdata;
    ...
    xlabel('正解IMCL占有率 [%]');
    ylabel('予測IMCL占有率 [%]');
    exportfig([dst_path,'\2018_11_25_pre_intensity'],'png',[300,250]);
    close gcf
    ...
    xlabel('correct IMCL percentage [%]');
    ylabel('predicted IMCL percentage [%]');
    exportfig([dst_path,'\2018_11_25_pre_intensity_eng'],'png',[300,250]);
    close gcf
    ...
    xlabel('correct IMCL percentage [%]');
    ylabel('predicted IMCL percentage [%]');
    exportfig([dst_path,'\2018_11_25_detected_intensity_eng'],'png',[300,250]);
    close gcf    
