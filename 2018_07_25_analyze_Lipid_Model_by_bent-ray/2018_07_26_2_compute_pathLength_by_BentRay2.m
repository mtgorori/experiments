...
% 屈折率分布
n = baseSoundSpeed./v_dist;%表示用
h = ones(100,100)/(100^2);
n = filter2(h,n);
n_cal = n';%計算用
% 媒質表示1(音速分布)
figure;
...
for ii = 1:t_num/2
    ...
    num_angle = 1;%射出角の総数(初期化)：ループごとに増加する
    num_trial = 0;%探索角度が広がっていく．
    ...
    initial_angle = 2*pi + atan2(t_pos(2,ind_fastest)-t_pos(2,ii) , t_pos(1,ind_fastest)-t_pos(1,ii));
    initial_angle0 = initial_angle;
    while(1)%走査角度内挿ループ
        for ind_angle = 1:num_angle
            ...
            while(1)%音線作成ループ
                ...
                if (ix <=75  || ix >= cell_num-75 || jy <=75 || jy >= cell_num-75)%音線が想定外の領域に進出した場合，情報を破棄してループし直す．
                    clear x y
                    break
                end
        ...
        initial_angle = linspace(initial_angle0-pi/2,initial_angle0+pi/2,num_angle);
    end
end