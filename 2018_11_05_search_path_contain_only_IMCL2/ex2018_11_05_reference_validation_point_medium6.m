...
point_maxAmp_in_mask = zeros(num_echo_receiver,num_depth,length(rate_IMCL));%マスク処理後のRFデータで振幅最大のサンプル点情報(包絡線処理後)
point_maxRF_in_mask = zeros(num_echo_receiver,num_depth,length(rate_IMCL));%マスク処理後のRFデータで振幅最大のサンプル点情報(包絡線処理前)
...
for kk = 1:length(rate_IMCL)
    ...
    for ii = 1:num_depth
        ...
        [~,point_maxRF_in_mask(:,ii,kk)] = max(abs(focused_rfdata_masked(:,:,ii,kk)),[],1);
        for jj = 1:length(target_element)
            %受信ビームフォーミング（整相加算のため）
