...
for mm = 1:num_lateral
    ...
    for ll = 1:num_medium
        ...
        for kk = 1:num_rate_IMCL
            ...
            if kk == 1
                ...
                for ii = 9:num_depth
                    ...
                    %受信ビームフォーカシング
                    focal_signal_total(ii,kk,ll,mm) =  receive_focus_wrong(focused_rfdata,target_element,reference_point);
                end
                %% 反射強度が最大の焦点位置を探索．
                ...
                %% 出力データの保存
                dst_path = sprintf('H:/result/2018_12_03_DAS_correct/case26/wrong/lateral%0.1fmm/true%d_assumption%d',...
        ...
        for kk = 1:num_rate_IMCL
            ...
            for ii = detected_boundary(1,ll,mm)
                ...
                focused_rfdata = transmit_focus(rfdata,target_element,num_sample,delay_time_all,num_echo_receiver);
                %受信ビームフォーカシング
                focal_signal(kk,ll,mm) =  receive_focus_wrong(focused_rfdata,target_element,reference_point);
            end
                %% 参照点と波面との形状相関を求める．（相互相関法）
            %RFデータマスキング（均質性評価のため)
            min_reference = min(reference_point) - 5;
    ...
    dst_path = sprintf('H:/result/2018_12_03_DAS_correct/case26/wrong/lateral%0.1fmm/',focal_point(1,9)*1e3);
...
%% すべての変数を保存
savefilename = sprintf('H:/result/2018_12_03_DAS_correct/case26/wrong/total_result.mat');
save(savefilename);