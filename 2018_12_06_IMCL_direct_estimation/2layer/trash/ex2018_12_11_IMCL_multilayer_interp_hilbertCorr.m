...
% IMCL割合を0 %に固定する．
% 波面遅延プロファイルにより音速推定
% 素子間受信波時間差は解析信号の正規化相互相関を用いる
% 内挿処理により解析用サンプリング数を実信号のサンプリング数の４倍にする
%%%%%%%%%%%%%%%%%%%%

%% 初期設定（共通）
clear
dst_path = sprintf('H:/result/2018_12_06_IMCL_direct_estimation/2layer/2018_12_11_rawsignal_hilbert_corr');
...
for mm = 1:num_medium
    ...
    hilb_interp_rfdata = hilbert(interp_rfdata);
    ...
    focused_rfdata_mask = mask_rfdata .* hilb_interp_rfdata;
    ...
    focused_rfdata_reference = reference_rfdata .* hilb_interp_rfdata;
    % 波面形状推定
    ...
    [~,delay_offset] = findpeaks(abs(focused_rfdata_mask(:,50)),'NPeaks',1,'SortStr','descend');
