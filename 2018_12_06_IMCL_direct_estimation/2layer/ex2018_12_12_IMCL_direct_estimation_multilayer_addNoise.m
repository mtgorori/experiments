...
%% 初期設定（共通）
clear
dst_path = sprintf('H:/result/2018_12_06_IMCL_direct_estimation/2layer/2018_12_12_Noise_signal_corr/10dB');
...
for mm = 1:num_medium
    ...
    rfdata = addnoise_to_rfdata(rfdata,10);
    %% フォーカシング
    %駆動素子の決定
    ...
    T = table(t_pos(1,target_element).',(delay_profile*kgrid.dt).^2.',f,(delay_profile*kgrid.dt).^2.'-f,'VariableNames',{'X','Y','Fit','FitError'});
    ...
    figure;
    imagesc(t_pos(1,:)*1e3,kgrid.t_array*1e9,abs(hilbert(focused_rfdata)));
    hold on
    scatter(t_pos(1,(min(target_element):max(target_element)))*1e3,delay_profile*kgrid.dt*1e9,'blue','filled');
    ...
    ylim([min_reference*kgrid.dt*1e9 max_mask*kgrid.dt*1e9])
    colormap(bone);
    colorbar;
    caxis([0 max(max(abs(hilbert(focused_rfdata_mask))))])
    savefilename = sprintf('/rfdata_detail_interp_Noise_signal_%0.1fmm',focal_depth(mm)*1e3);
    ...
    axis tight;
    xlim([t_pos(1,(min(target_element))-1)*1e3 t_pos(1,(max(target_element)+1))*1e3])
    ylim([0 round(max_mask*kgrid.dt*1e9*1.5)])
    colormap(bone);
    colorbar;
    caxis([0 max(max(abs(hilbert(focused_rfdata_mask))))])
    savefilename = sprintf('/rfdata_whole_interp_Noise_signal_%0.1fmm',focal_depth(mm)*1e3);
    savefig([dst_path,savefilename,'.fig'])
    exportfig([dst_path,savefilename],'png',[400,400])
    figure;
    ...
    title(titlename);
    savefilename = sprintf('/aberration_interp_Noise_signal_%0.1fmm',focal_depth(mm)*1e3);
    savefig([dst_path,savefilename,'.fig'])
...
xlabel('boundary depth[mm]');
ylabel('RMS error[ns]');
savefilename = sprintf('/ave_aberration_interp_Noise_signal');
savefig([dst_path,savefilename,'.fig'])
exportfig([dst_path,savefilename],'png',[300,250])
save([dst_path,'\2018_12_12_multi_layer_variable']);